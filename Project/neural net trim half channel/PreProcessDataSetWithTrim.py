...
def load_and_process_training_Data(targ_f_train, nontarg_f_train):
    ...
    if os.path.exists(targ_f_train)and os.path.exists(nontarg_f_train):
        ...
        # trim channel index
        channelLength = targEvent_train.shape[1]
        channelFirstHalfLength = channelLength//2
        # trim the last 4 channel for targ
        channelFirstHalf_targ = np.delete(targEvent_train,np.s_[channelFirstHalfLength:channelLength],1)
        targEvent_train = channelFirstHalf_targ
        # trim the last 4 channel for non_targ
        channelFirstHalf_nontarg = np.delete(nontargetEvent_train,np.s_[channelFirstHalfLength:channelLength],1)
        nontargetEvent_train = channelFirstHalf_nontarg
